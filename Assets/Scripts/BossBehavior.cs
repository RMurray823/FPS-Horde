...
public class BossBehavior:BaseEnemyCharacter {
		...
		switch (bossState) {
			...
			case "Death":
				...
				if (animDie) {
					...
					//Stop bosses movement and rotation
				}
				break;
			default:
	...
	private void Chasing() {
		...
		if (target != null) {
			if (Vector3.Distance(nav.transform.position, target.transform.position) > (nav.stoppingDistance + 9)) {
